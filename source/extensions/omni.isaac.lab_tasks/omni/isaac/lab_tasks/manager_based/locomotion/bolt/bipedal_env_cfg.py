...
class MySceneCfg(InteractiveSceneCfg):
    ...
    # robots
    robot: ArticulationCfg = BOLT_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
@configclass
class BoltFlatEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the locomotion velocity-tracking environment."""

    # Scene settings
    scene: MySceneCfg = MySceneCfg(num_envs=12000, env_spacing=2.5)
    # Basic settings
...
@configclass
class BoltFlatEnvCfg_PLAY(BoltFlatEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
@configclass
class BoltRoughEnvCfg(BoltFlatEnvCfg):
    curriculum: CurriculumCfg = CurriculumCfg()

    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        self.scene.num_envs = 8000
        self.scene.terrain.terrain_type = "generator"
        self.scene.terrain.terrain_generator = ROUGH_TERRAINS_CFG

        self.scene.terrain.terrain_generator.sub_terrains["pyramid_stairs"].step_height_range=(0.01, 0.1)
        self.scene.terrain.terrain_generator.sub_terrains["pyramid_stairs_inv"].step_height_range=(0.01, 0.1)
        self.scene.terrain.terrain_generator.sub_terrains["boxes"].grid_height_range=(0.01, 0.1)

        self.terminations.minimum_height = None
        self.rewards.base_height_l2 = None


@configclass
class BoltRoughEnvCfg_PLAY(BoltFlatEnvCfg_PLAY):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        self.scene.terrain.terrain_type = "generator"
